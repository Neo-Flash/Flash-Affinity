...
class Ligand_Protein_Dataset(InMemoryDataset):
    def __init__(self, root, data_dir, affinity_file, dataset_fraction = 1.0, transform = None, pre_transform = None):
    ...
    def process(self):
        ...
        # Get list of all valid directories (excluding hidden directories)
        all_dirs = [d for d in os.listdir(self.data_dir) if not d.startswith(".")]
        total_dirs = len(all_dirs)
        num_dirs_to_process = int(total_dirs * self.dataset_fraction)
        
        print(f"Processing {num_dirs_to_process} out of {total_dirs} directories ({self.dataset_fraction*100:.1f}% of dataset)")

        for i in tqdm(range(num_dirs_to_process)):
            complex_folder_name = all_dirs[i]
